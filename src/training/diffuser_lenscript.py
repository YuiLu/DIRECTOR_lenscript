...
class Diffuser(L.LightningModule):
    def __init__(
        ...
        first_frame_guidance_weight: float,
        use_first_frame_condition: bool, 
        ...
        **kwargs,
        ...
        self.first_frame_guidance_weight = first_frame_guidance_weight
        self.use_first_frame_condition = use_first_frame_condition
                    ...
                    if param in optimizer.state:
                        for key, value in optimizer.state[param].items():
                            if torch.is_tensor(value) and value.device.type != 'cuda':
                                optimizer.state[param][key] = value.to(self.device)
    ...
    def training_step(self, batch, batch_idx):
        ...
        first_frame = data[:, :, 0].clone() if self.use_first_frame_condition else None
        ...
        if len(self.modalities) > 0:
            ...
            self.first_frame_index = len(cond_data) - 1 if first_frame is not None else None
            conds["first_frame"] = first_frame
        else:
            cond_data = None
            self.first_frame_index = None

        # print(f"Shape of first frame cond: {cond_data[self.first_frame_index].shape if self.first_frame_index is not None else 'None'}")
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if len(self.modalities) > 0:
            ...
            if first_frame is not None:
                cond_data.append(first_frame)
                self.first_frame_index = len(cond_data) - 1
            else:
                self.first_frame_index = None
    ...
    def predict_step(self, batch):
        ...
        if "custom_first_frame" in batch and self.use_first_frame_condition:
            first_frame = batch["custom_first_frame"]
        else:
            first_frame = None
        ...
        if len(self.modalities) > 0:
            ...
            if first_frame is not None:
                cond_data.append(first_frame)
                first_frame_index = len(cond_data) - 1
            for cond in cond_k:
                cond_name = cond.replace("_feat", "")
                if isinstance(batch[f"{cond_name}_raw"], dict):
                    for cond_name_, x in batch[f"{cond_name}_raw"].items():
    ...
    def sample(
        ...
        generations = self.edm_sampler(
            ...
            first_frame_guidance_weight=self.first_frame_guidance_weight,
    ...
    def edm_sampler(
        ...
        has_first_frame = has_class_cond and first_frame_index is not None
        ...
        for i, (t_cur, t_next) in enumerate(
            ...
            if has_class_cond:
                if has_first_frame:
                    full_cond = [c.clone() for c in class_labels]
                    text_only_cond = [c.clone() for c in class_labels]
                    text_only_cond[first_frame_index] = torch.zeros_like(text_only_cond[first_frame_index])
                    no_cond = [torch.zeros_like(c) for c in class_labels]
                    
                    batch_x = torch.cat([x_hat, x_hat, x_hat], dim=0)
                    batch_t = torch.cat([t_hat.expand(bs)] * 3, dim=0)
                    batch_mask = torch.cat([bool_mask] * 3, dim=0)
                    
                    batch_conds = []
                    for j in range(len(class_labels)):
                        batch_conds.append(torch.cat([
                            full_cond[j], text_only_cond[j], no_cond[j]
                        ], dim=0))
                    
                    results = net(batch_x, batch_t, y=batch_conds, mask=batch_mask)
                    cond_full, cond_text_only, uncond = results.chunk(3, dim=0)
                    
                    text_influence = (cond_text_only - uncond) * guidance_weight
                    first_frame_influence = (cond_full - cond_text_only) * first_frame_guidance_weight
                    denoised = uncond + text_influence + first_frame_influence
            ...
            if i < num_steps - 1:
                if has_class_cond:
                    if has_first_frame:
                        ...
                        batch_x = torch.cat([x_next, x_next, x_next], dim=0)
                        batch_t = torch.cat([t_next.expand(bs)] * 3, dim=0)
                        batch_mask = torch.cat([bool_mask] * 3, dim=0)
                        ...
                        denoised = uncond + (cond_text_only - uncond) * guidance_weight + \
                                (cond_full - cond_text_only) * first_frame_guidance_weight
